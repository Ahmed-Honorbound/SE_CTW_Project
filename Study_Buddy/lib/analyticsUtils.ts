...
export function sessionDurationSeconds(session: { started_at: string; ended_at: string | null }): number {
...
export function filterCompletionsInWindow(
  records: Array<{ completed_at: string; outcome: string }>,
  windowStart: Date,
  windowEnd: Date
): Array<{ completed_at: string; outcome: string }> {
  const start = windowStart.getTime();
  const end = windowEnd.getTime();
...
export function computeWeeklyStats(data: { tasks: Array<any> }, weekStart: Date): WeeklyStats {
  ...
  if (Object.keys(taskSeconds).length > 0) {
    ...
    const task = data.tasks.find((t: any) => t.id === topId);
...
export function computeMonthlyStats(data: { tasks: Array<any> }, month: number, year: number): MonthlyStats {
  ...
  return {
    ...
    mostTimeSubject,
    leastTimeSubject,
    subjectMostCompleted,
    subjectMostOverdue,
    subjectMostAheadOfTime,
  };
}

export function computeSuggestions(data: { tasks: Array<any> }): Suggestions {
  ...
  const avoidSubjects = Object.entries(subjectOutcomes)
    .filter(([s]) => !focusSet.has(s) && s[1].completed >= 3 && s[1].overdue / s[1].completed > 0.7)
    .sort((a, b) => (b[1].overdue / b[1].completed) - (a[1].overdue / a[1].completed))
    .map(([s]) => s);
